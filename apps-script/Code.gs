...
var CHECKIN_HEADERS = ['報到時間', '簽退時間', '人事號', '院區', '單位', '姓名', '職稱'];

var SURVEY_SHEET_NAME = '工作坊滿意度調查';
var SURVEY_HEADERS = ['填答時間',
...
function findCheckinRow(sheet, empno) {
  ...
  for (var i = 0; i < values.length; i++) {
    if (String(values[i][2]).trim() === target) { // 人事號在第 3 欄
      return { rowIndex: i + 2, checkin: values[i][0], checkout: values[i][1], name: values[i][5] };
    }
  }
  return null;
}
...
function handleCheckin(ss, data) {
  ...
  if (row && !row.checkin) {
    ...
    csheet.appendRow([ts, '', empno, reg.campus, reg.dept, reg.name, reg.title]);
  }
